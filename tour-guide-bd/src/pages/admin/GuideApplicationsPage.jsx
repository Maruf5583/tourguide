...
export default function GuideApplicationsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      guideApi.getApplications({
        // status undefined পাঠালে backend সব দেখাবে
  ...
  const removeMutation = useMutation({
    // ✅ Fix: { reason } object পাঠাতে হবে, string না
    mutationFn: ({ id, reason }) => guideApi.removeGuide(id, { reason }),
    onSuccess: () => {
  ...
  // backend PaginatedList → { items, totalPages, hasPreviousPage, hasNextPage }
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          {STATUS_OPTIONS.map(s => (
            <button
              key={s.label}
              onClick={() => { setStatus(s.value); reset() }}
          ...
          {items.map(app => (
            <div key={app.id} className="card p-4 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {/* ✅ Fix: phoneNumber (not phone), createdAt (not appliedAt) */}
                <p className="text-xs text-gray-400">
                  {app.phoneNumber} · Applied {formatDate(app.createdAt)}
                </p>
              </div>
              ...
              <div className="flex gap-2 shrink-0">
                ...
                {app.status === STATUS.APPROVED && (
                  <button
                    onClick={() => { setRemoveTarget(app); setRemoveReason('') }}
                ...
                {isReviewable(app.status) && (
                  <>
                    <button
                      ...
                      className="p-2 rounded-lg bg-green-50 hover:bg-green-100 text-green-700 disabled:opacity-50"
                    >
                      <Check size={15} />
                    </button>
                    <button
                      ...
                      disabled={reviewMutation.isPending}
                      className="p-2 rounded-lg bg-red-50 hover:bg-red-100 text-red-700 disabled:opacity-50"
                    >
                      <X size={15} />
                    </button>
                  </>
          ...
          <div className="space-y-4 max-h-screen overflow-y-auto pr-1">
            ...
            <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-sm">
              {[
                ...
                ['Applied',       formatDate(selected.createdAt)],  // ✅ createdAt
              ].map(([label, value]) => (
            ...
            {/* Review actions (pending/under review) */}
        ...
        {removeTarget && (
          <div className="space-y-4">
            <p className="text-sm text-gray-600">
              <span className="font-semibold text-gray-900">{removeTarget.fullName}</span>-কে remove করতে চান?
            </p>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => removeMutation.mutate({
                  id: removeTarget.guideProfileId ?? removeTarget.id,
                  reason: removeReason,
                })}
